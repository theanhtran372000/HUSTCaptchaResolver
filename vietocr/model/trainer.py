    ...
    def train(self):
        ...
        for i in range(self.num_iters):
            ...
            if self.iter % self.print_every == 0:
                info = 'Iter: {:06d} - train loss: {:.5f} - lr: {:.2e} - load time: {:.2f} - gpu time: {:.2f}'.format(self.iter, 
            ...
            if self.valid_annotation and self.iter % self.valid_every == 0:
                ...
                info = 'Iter: {:06d} - valid loss: {:.5f} - acc full seq: {:.4f} - acc per char: {:.4f}'.format(self.iter, val_loss, acc_full_seq, acc_per_char)
                print(info)
                self.logger.log(info)

                if acc_full_seq > best_acc:
                    self.save_weights(self.export_weights)
                    print('==> Accuracy increase from {:.4f} to {:.4f}'.format(best_acc, acc_full_seq))
                    print('==> Save new weight to {}'.format(self.export_weights))
                    best_acc = acc_full_seq
        
        print('= ' * 10 + '\nBest accuracy achieved: {:.4f}\n'.format(best_acc) + '= ' * 10)   
        print('Done training!')     
